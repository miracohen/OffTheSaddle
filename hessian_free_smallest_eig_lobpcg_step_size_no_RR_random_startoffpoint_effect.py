# -*- coding: utf-8 -*-
"""
Created on Sat Apr  6 21:50:33 2024
...
def get_lambda(v, Av):
    return torch.matmul(Av, v.t()) / torch.matmul(v.t(), v)

def get_LnVn_LOBPCG(model, loss_function, optimizer, X, y, step_size, alpha, alpha_decay, use_preconditioner = False):
    ...
    i_v = torch.rand(model_size).to(DEVICE)
    i_v = i_v/torch.linalg.vector_norm(i_v, ord=2)
    i_v_prev = i_v.clone()
    t1 = time.time()
    time_elapsed = t1-t0
    ...
    i_w_prev = i_w.clone()
    t3 = time.time()
    i_v = (i_v_prev + alpha*i_w_prev).clone()
    i_v = i_v/torch.linalg.vector_norm(i_v, ord=2)
    i_Av = get_model_v_fd(model, loss_function, optimizer, X, y, i_v, None, None)
    i_l = get_lambda(i_v, i_Av)
    t4 = time.time()
    time_elapsed = t4-t3
    print('get_LnVn first naive iteration in {:.0f}m {:.0f}s'.format(time_elapsed // 60, time_elapsed % 60))
    i = 0
...
#     min_Vn = NV[0,:]
#     Avn = get_model_v_fd(model, loss_function, optimizer, X, y, min_Vn, None, preconditioner)
#     min_lambda_n = get_lambda(Avn, min_Vn)
#     for i in range(1, len(NV[:,0])):
#         v_i = NV[i,:]
#         Avi = get_model_v_fd(model, loss_function, optimizer, X, y, Avi, None, preconditioner)
#         lambda_i = get_lambda(Avi, v_i)
...
#     return min_Vn, min_lambda_n, R[-1][-1] #Vn, Ln, R[-1][-1]
...
def evaluate_smallest_eig_model(filepath):
    ...
    try:
        ...
        Vn_LOBPCG, Ln_LOBPCG, iN_LOBPCG, run_log = get_LnVn_LOBPCG(model, loss_function, optimizer, data_x, data_y, step_size, alpha=ALPHA, alpha_decay=ALPHA_DECAY, use_preconditioner=False)
        print("finished LOB")
...
def main():
    ...
    for model_path in MODEL_PATHS:
        ...
        for i in range(3):
            ...
            with pd.ExcelWriter(OUT_PATH_SUMMARY + "\\rand_3_start_offs_points_noRR_" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")+"ALPHA="+str(ALPHA)+"ALPHA_DECAY="+str(ALPHA_DECAY)+"_summary_res.xlsx") as writer:
