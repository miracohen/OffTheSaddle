...
# PATH_SAVE_MODEL = r"./runs/CIFAR10_RESNET18/SGD/LOG_" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")+"LR="+str(LR)+"WEIGHT_DECAY="+str(WEIGHT_DECAY)
...
def main():
    ...
    # optimizer = optim.AdamW(CIFAR10_RESNET18.model.parameters(), lr=LR, weight_decay=WEIGHT_DECAY)
    lr_scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=30, verbose=True)
    
    CIFAR10_RESNET18.set_criterion_optimizer_scheduler(criterion, optimizer, lr_scheduler)
    model, val_acc_history, epochs_data =  CIFAR10_RESNET18.train_model(EPOCHS, phases = ['train'])#, 'val'])
