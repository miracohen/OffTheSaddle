...
import os 
import datetime
from CIFAR10_MLP_model_V import MLP, CIFAR10_MLP, CustomTensorDataset
...
def main():
    ...
    CIFAR10_MLP.init_model(3, 10)
    CIFAR10_MLP.load_data()
    
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.SGD(CIFAR10_MLP.model.parameters(), lr=LR)#, weight_decay=WEIGHT_DECAY)
    if OPTIMIZER_TYPE == 'ADAM':
        optimizer = optim.AdamW(CIFAR10_MLP.model.parameters(), lr=LR)#, weight_decay=WEIGHT_DECAY)
    lr_scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor = LR_REDUCE_FACTOR, patience=10, verbose=True)
    
    CIFAR10_MLP.set_criterion_optimizer_scheduler(OPTIMIZER_TYPE, criterion, optimizer, lr_scheduler)
    model, val_acc_history, epochs_data =  CIFAR10_MLP.train_model(EPOCHS, phases = ['train'])#, 'val'])
